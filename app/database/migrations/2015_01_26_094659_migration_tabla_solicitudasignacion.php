...
class MigrationTablaSolicitudasignacion extends Migration {

	public function up() {
		Schema::create('solicitudasignacion', function(Blueprint $table) {
			$table->increments('solicitudasignacionid');
			$table->integer('usuarioid')->unsigned();
			$table->integer('periodoid')->unsigned();
			$table->decimal('solicitado', 20, 5);
			$table->decimal('asignado', 20, 5)->nullable();
	...
	public function down() {
		Schema::drop('solicitudasignacion');
	}
